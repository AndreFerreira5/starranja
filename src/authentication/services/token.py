...
class RefreshTokenService:
    """
    Service for handling Refresh Token lifecycle.
    Requires a database session and password service instance per request.
    """

    def __init__(self, db: AsyncSession, password_service):
        self.db = db
        self.password_service = password_service
        self.REFRESH_TOKEN_EXPIRE_DAYS = 7  # Configurable via settings if needed

    async def generate_refresh_token(self, user_id: UUID) -> str:
        """
        Generates a refresh token, stores the hash in DB, and returns 'uuid:secret'.
        """
        # 1. Generate secure random string
        secret_token = secrets.token_urlsafe(64)

        # 2. Hash it (Argon2 is salted, so we can't search by this hash later)
