...
class PasswordService:
    ...
    def _validate_password(self, password: str) -> None:
        ...
        if len(password) < settings.MIN_PASSWORD_LENGTH:
            raise InvalidPasswordError(f"Password must be at least {settings.MIN_PASSWORD_LENGTH} characters long")

        if len(password) > settings.MAX_PASSWORD_LENGTH:
            raise InvalidPasswordError(f"Password must not exceed {settings.MAX_PASSWORD_LENGTH} characters")
